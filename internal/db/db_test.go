...
// The no-separator form (#205) and the part suffix (#204) compose: TS36.521-1
// is a citation of the multi-part spec TS 36.521-1.
func TestExtractReferences_NoSeparatorMultiPart(t *testing.T) {
	content := "The requirements are defined in TS36.521-1 clause 5.2."

	refs := ExtractReferences("TS 24.229", "5.1", content, nil)
	if len(refs) != 1 {
		t.Fatalf("expected 1 reference, got %d: %+v", len(refs), refs)
	}
	if refs[0].TargetSpec != "TS 36.521-1" || refs[0].TargetSection != "5.2" {
		t.Errorf("expected TS 36.521-1 clause 5.2, got %+v", refs[0])
	}
}
